package net.angelicaether.eden_expanded.block.custom;
...
// I can't be fucking bothered to figure out what this shit means, I just copied the grass block type and edited it
public class GritsoilGrassBlock extends SpreadableBlock implements Fertilizable {
    public GritsoilGrassBlock(AbstractBlock.Settings settings) {
        super(settings);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            world.setBlockState(pos, ModBlocks.GRITSOIL.getDefaultState());
        } else {
            if (world.getLightLevel(pos.up()) >= 9) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (world.getBlockState(blockPos).isOf(ModBlocks.GRITSOIL) && canSpread(blockState, world, blockPos)) {
